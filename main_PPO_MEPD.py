...
def save_params_to_json(params, filename_prefix="params",output_path='data'):
    ...
    src_file='main_PPO_MEPD.py'
    dst_file=f'{output_path}/{src_file}'
    shutil.copy2(src_file, dst_file)
    src_file='PPO_MEPD.py'
    dst_file=f'{output_path}/{src_file}'
    shutil.copy2(src_file, dst_file)
...
async def main(args):
    ...
    # 实验参数设置
    fontsize=16
    # r_values = [4.3,5.2]#[4.5,4.6,4.7,4.8,4.9,5.0,5.1]#[3.6, 3.8, 4.7, 5.0, 5.5, 6.0] #[3.0, 5.0, 7.0, 9.0]  # 公共物品乘数
    ...
    elif args.epochs==5000:
        xticks=[0, 1, 10, 100, 1000, 5000]
    ...
    if args.expert_paths is not None:
        output_path=f'data/PPO_MEPD_{formatted_time}_me_{len(args.expert_paths)}_q{str(args.question)}_e_{args.epochs}_L_{args.L_num}_a_{args.alpha}_g_{args.gamma}_ce_{args.clip_epsilon}_gl_{args.gae_lambda}_p_{args.ppo_epochs}_b_{args.batch_size}_delta_{args.delta}_rho_{args.rho}_eta_{args.eta}_tau_{args.tau}_beta_{args.beta}_seed_{args.seed}'
    else:
        r_values = [round(i * 0.1, 1) for i in range(50, 61)]
        output_path=f'data/PPO_expert_{formatted_time}_q{str(args.question)}_e_{args.epochs}_L_{args.L_num}_a_{args.alpha}_g_{args.gamma}_ce_{args.clip_epsilon}_gl_{args.gae_lambda}_p_{args.ppo_epochs}_b_{args.batch_size}_delta_{args.delta}_rho_{args.rho}_seed_{args.seed}'
